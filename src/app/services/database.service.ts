...
export class DatabaseService {
	 ...
	 */
	callFunction<T, R>(functionName: string, data: T, options?: any): Observable<R> {
		const callable = httpsCallable<T, R>(this.functions, functionName, options);
	...
	/**
	 * Trigger action post generation for the current user
	 */
	triggerActionPosts(force: boolean = false): Observable<{ success: boolean; message: string; postsCreated: number; debugLogs?: string[] }> {
		return this.callFunction<{ type: string; force: boolean }, { success: boolean; message: string; postsCreated: number; debugLogs?: string[] }>(
			'triggerFeedPost',
			{ type: 'action', force }
		);
	}
	...
	/**
	 * Trigger insight post generation for the current user
	 */
	triggerInsightPosts(force: boolean = false): Observable<{ success: boolean; message: string; postsCreated: number; debugLogs?: string[] }> {
		return this.callFunction<{ type: string; force: boolean }, { success: boolean; message: string; postsCreated: number; debugLogs?: string[] }>(
			'triggerFeedPost',
			{ type: 'insight', force }
		);
	 ...
	 */
	updateUserFCMToken(userId: string, token: string): Observable<void> {
