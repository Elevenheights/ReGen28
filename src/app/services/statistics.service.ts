...
export class StatisticsService {
  ...
  getUserDailyStats(options: {
    ...
    return this.db.callFunction<typeof options, UserDailyStats | UserDailyStats[]>('getUserDailyStatsFunction', options).pipe(
      map(result => {
  ...
  getTrackerDailyStats(trackerId: string, options: {
    ...
    return this.db.callFunction<any, TrackerDailyStats[]>('getTrackerDailyStatsFunction', {
      trackerId,
      ...options
    }).pipe(
      map(stats => {
  ...
  getJournalDailyStats(options: {
    ...
    return this.db.callFunction<typeof options, JournalDailyStats[]>('getJournalDailyStatsFunction', options).pipe(
      map(stats => {
        this.setCachedData(cacheKey, stats);
        return stats;
      }),
  ...
  getWeeklyMoodTrend(weeksBack: number = 4): Observable<WeeklyMoodTrend[]> {
    ...
    return this.db.callFunction<{ weeksBack: number }, WeeklyMoodTrend[]>('getWeeklyMoodTrend', { weeksBack }).pipe(
      map(trend => {
  ...
  getPerformanceInsights(days: number = 30): Observable<PerformanceMetrics> {
    ...
    return this.db.callFunction<{ days: number }, PerformanceMetrics>('getPerformanceInsights', { days }).pipe(
      map(insights => {
