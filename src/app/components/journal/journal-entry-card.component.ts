...
export class JournalEntryCardComponent {
	...
	getMoodBackground(mood?: number): string {
		if (!mood || mood < 1) return 'bg-neutral-100';

		if (mood <= 2) return 'bg-red-100';
		if (mood <= 4) return 'bg-orange-100';
		if (mood <= 6) return 'bg-yellow-100';
		if (mood <= 8) return 'bg-emerald-100';
		return 'bg-green-100';
	}
