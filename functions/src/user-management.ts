...
import { createWelcomeFeed } from './onboarding-helpers';
...
export const completeUserOnboarding = onCall({
	...
	try {
		...
		// 6. Create initial feed content (Welcome posts + first AI generation)
		await createWelcomeFeed(userId, profileData.displayName || 'Champion');
	...
	try {
		...
		if (openaiService.isAvailable()) {
			try {
				...
				const weather = location !== 'Unknown' ? await getWeatherForLocation(location) : 'Unknown';

				let prompt = `You are ${userName}'s personal wellness coach - fun, witty, and encouraging! We are in ${season} at ${location}. Weather: ${weather}.
...
- Take into account the likely WEATHER (${weather}) based on their Location (${location}) and Season (${season}) when suggesting outdoor activities.
